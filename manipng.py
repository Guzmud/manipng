...
def chunkeater(data):
    ...
    k = 0
    while k<len(data):
        tlen = int(''.join([hex(ord(x))[2:].zfill(2) for x in data[k:k+4]]),16)
        k += 4
        ...
        tcrc = int(''.join([hex(ord(x))[2:].zfill(2) for x in data[k:k+4]]),16)
        k += 4
...
print "Data size after the png trailer : "+str(len(data)-(data.find(tag2)+len(tag2)+8))
...
theader = ''.join([hex(ord(k))[2:].zfill(2) for k in data[:len(header)/2]]).lower()
...
print mdata
clist = list(set([k[0] for k in mdata]))
ulist = []
for k in clist:
    if k in pngchunks:
        print "\t "+k+" : "+pngchunks[k],
        ...
        ulist.append(k)
elist = [x for x in mdata if (x[0] == 'IDAT' and x[1] == 0)]
print ""
...
print "Looking for interesting data ..."
...
# check against the norm (size of ancillary chunks, scale of values, position of ancillary, keywords in tEXt and zTXt, etc.)
